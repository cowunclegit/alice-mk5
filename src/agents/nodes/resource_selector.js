...
export const resourceSelector = async (state, config) => {
  ...
  let selected = ['core.resource']; // Always include core
  ...
  try {
    ...
    await logger.error(`ResourceSelector: Failed to read manifest: ${e.message}. Falling back to filename matching.`);
    // Fallback logic could go here, but manifest is the primary way now.
  }
